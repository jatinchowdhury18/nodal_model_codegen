...
static void compute_stage1 (const float* const* input, float** output, int num_channels, int num_samples, Params_stage1 params, State_stage1* state, float sample_rate)
{
    ...
    for (int ch = 0; ch < num_channels; ++ch)
    {
        ...
        for (int n = 0; n < num_samples; ++n)
        {
            ...
            const auto _D1_t6 = (1.0f / (gR1 + gR2));
            ...
            for (int newton_iter = 0; newton_iter < newton_max_iter; ++newton_iter)
            {
                ...
                const auto _D1_t1 = ((((_D1_t2 - _D1_t9) - _D1_t10) * _D1_t6) + ((zR3C1 + _D1_t2) / gR3C1));
                const auto _D1_t0 = (_D1_t1 + vD1);
                const auto res_vD1 = (-_D1_t0);
                const auto delta_vD1 = (-(_D1_t0 / (((_D1_t7 * _D1_t6) + (_D1_t7 / gR3C1)) + 1.0f)));
...
static float reset_stage1 (Params_stage1 params, State_stage1* state, int num_channels, float sample_rate, float vi_dc = 0.0f)
{
    ...
    // --- Newton-Raphson solve: D1
    const auto _D1_t6 = (1.0f / ((gR1 + gR2) + (1.0f / 1000000000.0f)));
    ...
    for (int newton_iter = 0; newton_iter < 10000; ++newton_iter)
    {
        ...
        const auto _D1_t3 = (_D1_t4 - 1.0f);
        const auto _D1_t2 = (D1N914_Is * _D1_t3);
        const auto _D1_t1 = ((((((_D1_t2 - _D1_t8) / R3) - _D1_t9) * R3) * _D1_t6) + (_D1_t2 * R3));
        const auto _D1_t0 = (_D1_t1 + vD1);
        const auto res_vD1 = (-_D1_t0);
        const auto delta_vD1 = (-(_D1_t0 / ((((_D1_t7 / D1N914_vt) * _D1_t6) + ((_D1_t7 * R3) / D1N914_vt)) + 1.0f)));
    ...
    for (int ch = 0; ch < num_channels; ++ch)
    {
        ...
        for (int n = 0; n < num_samples; ++n)
        {
            ...
            for (int newton_iter = 0; newton_iter < newton_max_iter; ++newton_iter)
            {
                ...
                const auto _D1x_t2 = (D1N914_Is * _D1x_t3);
                const auto _D1x_t1 = (((_D1x_t2 - _D1x_t8) / gR1x) + ((zR3xC1x + _D1x_t2) / gR3xC1x));
                const auto _D1x_t0 = (_D1x_t1 + vD1x);
                const auto res_vD1x = (-_D1x_t0);
                const auto delta_vD1x = (-(_D1x_t0 / (((_D1x_t6 / gR1x) + (_D1x_t6 / gR3xC1x)) + 1.0f)));
...
static float reset_stage2 (Params_stage2 params, State_stage2* state, int num_channels, float sample_rate, float vo1_dc = 0.0f)
{
    ...
    // --- Newton-Raphson solve: D1x
    const auto _D1x_t7 = (1.0f / 1000000000.0f);
    const auto _D1x_t8 = (1.0f / ((1.0f / R3x) + _D1x_t7));
    const auto _D1x_t6 = (1.0f / (gR1x + _D1x_t7));
    ...
    for (int newton_iter = 0; newton_iter < 10000; ++newton_iter)
    {
        ...
        const auto _D1x_t2 = (D1N914_Is * _D1x_t3);
        const auto _D1x_t1 = (((_D1x_t2 - _D1x_t11) * _D1x_t6) + (_D1x_t2 * _D1x_t8));
        const auto _D1x_t0 = (_D1x_t1 + vD1x);
        const auto res_vD1x = (-_D1x_t0);
        const auto delta_vD1x = (-(_D1x_t0 / (((_D1x_t9 * _D1x_t6) + (_D1x_t9 * _D1x_t8)) + 1.0f)));
