...
static float limit_junction_voltage(float v_new, float v_old, float vt, float vcrit) {
    if (v_new > vcrit && fabsf(v_new - v_old) > 2 * vt) {
        if (v_old > 0) {
            ...
            v_new = arg > 0 ? v_old + vt * logf(arg) : vcrit;
        } else {
            v_new = vt * logf(v_new / vt);
        }
    ...
    for (int ch = 0; ch < num_channels; ++ch)
    {
        ...
        for (int n = 0; n < num_samples; ++n)
        {
            ...
            for (int newton_iter = 0; newton_iter < newton_max_iter; ++newton_iter)
            {
                ...
                const float _Q1_t9 = expf(_Q1_t10);
                const float _Q1_t13 = expf(_Q1_t14);
            ...
            const float _t4 = expf((vBEQ1 / Q2N5089_vt));
            const float _t5 = expf((vBCQ1 / Q2N5089_vt));
...
static float reset (Params params, State* state, int num_channels, float sample_rate, float vi_dc)
{
    ...
    for (int newton_iter = 0; newton_iter < 10000; ++newton_iter)
    {
        ...
        const float _Q1_t8 = expf(_Q1_t9);
        const float _Q1_t12 = expf(_Q1_t13);
    ...
    const float zC1 = (gC1 * (vi - (((gR1 * VCC) - (Q2N5089_Is * (((expf((vBEQ1 / Q2N5089_vt)) - 1.0f) / Q2N5089_BetaF) + ((expf((vBCQ1 / Q2N5089_vt)) - 1.0f) / Q2N5089_BetaR)))) / ((gR1 + gR2) + (1.0f / 1000000000.0f)))));
    const float zC2 = (-((gC2 * ((Q2N5089_Is * ((expf((vBEQ1 / Q2N5089_vt)) - expf((vBCQ1 / Q2N5089_vt))) - ((expf((vBCQ1 / Q2N5089_vt)) - 1.0f) / Q2N5089_BetaR))) - (VCC * gRC))) / (gRC + (1.0f / 1000000000.0f))));
