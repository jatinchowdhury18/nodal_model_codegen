...
static void compute (const float* const* input, float** output, int num_channels, int num_samples, Params params, State* state, float sample_rate)
{
    ...
    for (int ch = 0; ch < num_channels; ++ch)
    {
        ...
        auto vCBQ1_prev = state[ch].vCBQ1_prev;
        auto vEBQ1 = state[ch].vEBQ1;
        auto vEBQ1_prev = state[ch].vEBQ1_prev;
        for (int n = 0; n < num_samples; ++n)
        {
            const auto vi = input[ch][n];

            { const auto _prev_step = vCBQ1 - vCBQ1_prev; vCBQ1_prev = vCBQ1;
vCBQ1 = limit_junction_voltage(vCBQ1 + (_prev_step), vCBQ1, Q2N5087_vt, vcrit_Q2N5087_vt);
            }
            { const auto _prev_step = vEBQ1 - vEBQ1_prev; vEBQ1_prev = vEBQ1;
