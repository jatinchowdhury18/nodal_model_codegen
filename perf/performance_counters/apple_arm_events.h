...
struct performance_counters {
  ...
  double instructions;
  double l1d_load_misses;
  double l1d_store_misses;
  // Load/store-unit uop counts -- every load/store executed, hit or miss.
  // Unlike the miss counters above, this actually catches register-pressure
  // spill traffic: a spill store + reload load hits L1 almost every time
  // (it's hot, freshly-written data), so it wouldn't show up as a miss at
  // all. See LD_UNIT_UOP/ST_UNIT_UOP in profile_events below.
  double ld_unit_uops;
  double st_unit_uops;
  performance_counters(uint64_t c, uint64_t b, uint64_t m, uint64_t i,
                        uint64_t l1dl = 0, uint64_t l1ds = 0,
                        uint64_t ldu = 0, uint64_t stu = 0)
      : cycles(c), branches(b), missed_branches(m), instructions(i),
        l1d_load_misses(l1dl), l1d_store_misses(l1ds),
        ld_unit_uops(ldu), st_unit_uops(stu) {}
  performance_counters(double c, double b, double m, double i,
                        double l1dl = 0, double l1ds = 0,
                        double ldu = 0, double stu = 0)
      : cycles(c), branches(b), missed_branches(m), instructions(i),
        l1d_load_misses(l1dl), l1d_store_misses(l1ds),
        ld_unit_uops(ldu), st_unit_uops(stu) {}
  performance_counters(double init)
      : cycles(init), branches(init), missed_branches(init),
        instructions(init), l1d_load_misses(init), l1d_store_misses(init),
        ld_unit_uops(init), st_unit_uops(init) {}
  ...
  inline performance_counters &operator-=(const performance_counters &other) {
    ...
    l1d_store_misses -= other.l1d_store_misses;
    ld_unit_uops -= other.ld_unit_uops;
    st_unit_uops -= other.st_unit_uops;
    return *this;
  }
  inline performance_counters &min(const performance_counters &other) {
    ...
    l1d_load_misses = other.l1d_load_misses < l1d_load_misses
                          ? other.l1d_load_misses
                          : l1d_load_misses;
    l1d_store_misses = other.l1d_store_misses < l1d_store_misses
                          ? other.l1d_store_misses
                          : l1d_store_misses;
    ld_unit_uops = other.ld_unit_uops < ld_unit_uops
                       ? other.ld_unit_uops
                       : ld_unit_uops;
    st_unit_uops = other.st_unit_uops < st_unit_uops
                       ? other.st_unit_uops
                       : st_unit_uops;
    return *this;
  }
  ...
  inline performance_counters &operator/=(double numerator) {
    ...
    l1d_load_misses /= numerator;
    l1d_store_misses /= numerator;
    ld_unit_uops /= numerator;
    st_unit_uops /= numerator;
    return *this;
  }
};
...
inline performance_counters operator-(const performance_counters &a,
  ...
  return performance_counters(a.cycles - b.cycles, a.branches - b.branches,
                              ...
                              a.l1d_load_misses - b.l1d_load_misses,
                              a.l1d_store_misses - b.l1d_store_misses,
                              a.ld_unit_uops - b.ld_unit_uops,
                              a.st_unit_uops - b.st_unit_uops);
}
...
static const event_alias profile_events[] = {
    ...
    // Apple does not expose a dedicated L2 data-cache-miss event through kpep
    // on A14/M1 (only L2 TLB miss events exist); L1D miss counters are the
    // finest-grained memory-stall signal available here.
    {"l1d-load-misses",
     {
         "L1D_CACHE_MISS_LD", // Apple A7-A15: loads that missed the L1 Data Cache
     }},
    {"l1d-store-misses",
     {
         "L1D_CACHE_MISS_ST", // Apple A7-A15: stores that missed the L1 Data Cache
     }},
    // Total load/store-unit uops executed, hit or miss -- unlike the miss
    // counters above, this is the register-pressure proxy: a spill
    // store+reload hits L1 almost every time (hot, freshly-written data),
    // so it wouldn't move the miss counters at all. Names come from
    // Instruments' PMU event list (reverse-engineered, not officially
    // documented by Apple) -- not yet cross-checked against a disassembly
    // load/store count on real hardware.
    {"ld-unit-uops",
     {
         "LD_UNIT_UOP", // Apple Firestorm/Icestorm (M1): uops executed on the load unit
     }},
    {"st-unit-uops",
     {
         "ST_UNIT_UOP", // Apple Firestorm/Icestorm (M1): uops executed on the store unit
     }},
};
...
struct AppleEvents {
  ...
  inline performance_counters get_counters() {
    ...
    return performance_counters{
        ...
        counters_0[counter_map[4]], counters_0[counter_map[5]],
        counters_0[counter_map[6]], counters_0[counter_map[7]]};
  }
};
